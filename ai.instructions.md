# Football Tracker - AI System Instructions

## Project Overview
- **Purpose:** An app to track weekly football players and record financial transactions (who paid, who owes money) for each participant.
...
- **Aesthetics:** Clean, minimalist layouts with generous white space, a consistent, high-quality color palette, and high-quality visuals.
- **Typography:** Thoughtful typography for hierarchy and readability.
- **Components & Styling:** Consistent UI components with depth (shadows, rounded corners).
- **Interactions:** Smooth micro-interactions, subtle animations, and intuitive feedback.
...
2. `transactions`:
   - `id` (uuid, primary key)
   - `player_id` (uuid, foreign key, nullable for kitty expenses/pitch bookings)
   - `amount` (numeric, positive for paid, negative for owed)
   - `date` (date)
   - `type` (text, restricted to: 'Bank Payment', 'Cash Payment', 'Game Fee', 'Kitty Expense', 'Pitch Booking')